...
export const ReferralTemplates = () => {
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const data = await communicationService.getTemplates(3);
      setTemplates(data || []);
                ...
                <List sx={{ p: 0 }}>
                  {templates.map((template, index) => (
                    <React.Fragment key={template.id || index}>
                      ...
                      >
                        <Typography sx={{ fontSize: '0.8rem', color: '#1E293B', flexGrow: 1 }}>{template.description || template.name}</Typography>
