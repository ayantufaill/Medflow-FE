...
const CollectionCodeCarrierTable = ({ loading, reportData }) => {
  return (
    <Box sx={{ border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden', mt: 2 }}>
      <TableContainer 
        id="collection-carrier-table" 
        elevation={0} 
        ...
        <Table size="small" sx={{ minWidth: 800 }}>
          <TableHead>
            <TableRow sx={{ '& th': { fontSize: '0.7rem', fontWeight: 700, backgroundColor: '#f8f9fa', py: 1, borderBottom: '1px solid #e2e8f0', color: '#1e293b' } }}>
              ...
              <TableCell align="right">
                <Box sx={{ display: 'inline-flex', alignItems: 'center' }}>
                  Average per Code <UnfoldMoreIcon className="no-print" sx={{ fontSize: 16, ml: 0.5, color: '#94a3b8' }} />
                </Box>
              </TableCell>
            </TableRow>
          </TableHead>
            ...
            {reportData.length === 0 ? (
              ...
              reportData.map((row, idx) => (
                ...
                >
                  ...
                  <TableCell align="right">${(row.avgPerCode || 0).toFixed(2)}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
...
export default CollectionCodeCarrierTable;
