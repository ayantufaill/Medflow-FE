...
const CoronalToothStructure = ({ 
  ...
  const handleApplyCoronalRadiolucency = () => {
      ...
      selectedTeeth.forEach(num => {
        if (!updated[num]) {
          updated[num] = {
            ...
            surfaces: ['M', 'O/I'], // default surfaces
            depth: 'Limited to enamel',
            notes: []
          };
        } else if (!updated[num].findings.includes('Coronal radiolucency')) {
          updated[num] = {
            ...updated[num],
            findings: [...updated[num].findings, 'Coronal radiolucency'],
            surfaces: updated[num].surfaces || ['M', 'O/I'],
    ...
    // Set the first applied tooth as active, but do NOT clear selectedTeeth
    // to allow further batch modifications.
    ...
    setToothFindings(prev => {
      ...
      selectedTeeth.forEach(num => {
        if (!updated[num]) {
          ...
          updated[num] = {
            ...updated[num],
            findings: [...new Set([...(updated[num].findings || []), 'Coronal radiolucency'])],
            depth: depth
          };
        }
      });
    ...
    <Card sx={{ mb: 2, borderRadius: 2, border: '1px solid #e5e7eb', bgcolor: 'white', boxShadow: 'none', overflow: 'hidden' }}>
      <Box sx={{ 
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          <Typography sx={{ fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Coronal Tooth Structure</Typography>
        ...
        <Box sx={{ p: 1.5, ...(noFindings && { opacity: 0.4, pointerEvents: 'none', userSelect: 'none' }) }}>
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
            <Stack 
              ...
              spacing={0.5} 
              alignItems="center"
              onClick={handleApplyCoronalRadiolucency}
            ...
            <Stack direction="row" spacing={0.5} alignItems="center">
              <Box 
                ...
                title="Limited to enamel (Small Dot)"
              >
              ...
              <Box 
                ...
                title="Deep into dentin (Large Dot)"
              >
          ...
          <Stack direction="row" spacing={0.8} justifyContent="flex-end" flexWrap="wrap" useFlexGap sx={{ mb: 2, pr: 0.5 }}>
            {Object.entries(toothFindings)
              .filter(([, data]) => data && data.findings && data.findings.includes('Coronal radiolucency'))
              .map(([toothNum, data]) => {
