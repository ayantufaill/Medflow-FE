...
const ToothSelectionDialog = ({ open, onClose, activeSelectionCode, isToothSelected, onToothToggle }) => {
      ...
      <Box sx={{ position: 'relative', p: 2, minWidth: '400px' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1 }}>
            {topTeeth.map(t => {
              const isSelected = activeSelectionCode !== null && isToothSelected(t);
              const isQ = t.startsWith('Q');
              return (
                <Typography
                  key={t}
                  onClick={() => onToothToggle(t)}
                  sx={{ 
                    fontSize: '0.75rem', 
                    width: '24px', 
                    textAlign: 'center', 
