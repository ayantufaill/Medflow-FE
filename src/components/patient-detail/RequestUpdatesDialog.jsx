...
export default function RequestUpdatesDialog({
  ...
  return (
    <Dialog
      ...
      fullWidth
      sx={{ zIndex: 1400 }}
      PaperProps={{ sx: { width: '450px', maxWidth: '92vw', borderRadius: radius.lg, p: 0 } }}
    >
      <Box sx={{
        display: 'flex',
        alignItems: 'center',
      ...
      <DialogContent sx={{ p: 0 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', py: 1 }}>
          {[
            ...
            >
              <Checkbox
                ...
                size="small"
                disableRipple
                sx={{ p: 0, mr: 2, color: COLORS.BORDER, '&.Mui-checked': { color: COLORS.ACCENT } }}
              />
              <Box sx={{ flex: 1 }}>
                <Typography sx={{ fontSize: '14px', color: COLORS.TEXT_PRIMARY, fontWeight: 500 }}>
                  {label}
                </Typography>
              </Box>
          ...
          <Box sx={{ px: 3, py: 1.5, mt: 1, backgroundColor: COLORS.SURFACE_TINT, borderTop: `1px solid ${COLORS.BORDER}`, borderBottom: `1px solid ${COLORS.BORDER}` }}>
            <Typography sx={{ fontSize: '13px', fontWeight: 600, color: COLORS.TEXT_SECONDARY, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Custom Forms
            </Typography>
          </Box>
      ...
      <DialogActions sx={{ p: 2, borderTop: `1px solid ${COLORS.BORDER}`, backgroundColor: COLORS.SURFACE_CARD, borderBottomLeftRadius: radius.lg, borderBottomRightRadius: radius.lg, gap: 1 }}>
        <Button
          onClick={onClose}
          sx={{
            textTransform: 'none',
