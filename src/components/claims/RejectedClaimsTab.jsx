...
const RejectedClaimsTab = ({ onOpenEdit, onOpenAttach, onOpenPreview }) => {
  ...
  const { loading, changeStatus, voidAndRecreate, exportCSV, printPage, toggleHide } = useClaimActions(loadData);
  ...
  async function loadData() {
    ...
    try {
      const data = await claimService.getAllClaims({ tab: 'rejected' });
      const claimsData = (data.claims || []).map(c => mapClaimFields(c, 'rejected'));
  ...
  return (
    <Box>
      ...
      <StandardClaimsTable
        activeTab={2} // Rejected is index 2
...
export default RejectedClaimsTab;
