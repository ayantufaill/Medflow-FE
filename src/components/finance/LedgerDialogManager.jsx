...
const LedgerDialogManager = ({
  ...
  handleCourtesyCreditSave,
  editTarget,
  showUndoDialog,
  handleUndoCancel,
  handleUndoConfirm,
  showSimpleStatement,
  setShowSimpleStatement,
  showDetailedStatement,
  setShowDetailedStatement,
  showEditDeposit,
  ...
  <>
    ...
    {[
      ...
      >
        <DialogContent sx={{ p: 0 }}>
          <Component onClose={onClose} editTarget={adjItem || editTarget} />
        </DialogContent>
      </Dialog>
    ))}
    ...
    {[
      ...
      <Dialog
        ...
        sx={{
          zIndex: 130000,
          "& .MuiDialog-paper": { margin: 0, maxWidth: "100%" },
        }}
      >
        <DialogContent sx={{ p: 0, m: 0, bgcolor: "#f5f5f5" }}>
          <Component onClose={onClose} printItem={printItem} />
        </DialogContent>
      </Dialog>
    ))}
    ...
    <Menu
      ...
      onClose={() => setMagicStickAnchorEl(null)}
      PaperProps={{
        sx: {
        ...
        onSave={async (data) => {
          if (data.newFiles && data.newFiles.length > 0 && attachTarget?.id) {
            try {
              ...
              await claimService.uploadAttachments(
                attachTarget.id,
                data.newFiles,
              );
