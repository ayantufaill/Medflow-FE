...
const UserManagementView = () => {
  ...
  const usersByRole = roles.reduce((acc, role) => {
    ...
    acc[roleName] = users.filter((u) =>
      (u.roles || []).some((r) => (typeof r === 'string' ? r : r?.name) === roleName)
    );
    return acc;
  }, {});
  ...
  const hasAnyUsers =
    Object.values(usersByRole).some((arr) => arr.length > 0) || ungrouped.length > 0;
  ...
  return (
    <Box>
      ...
      {/* Top-right: Add User + show inactive */}
