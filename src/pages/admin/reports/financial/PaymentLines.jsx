...
const PaymentLines = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    // Filter by archived
    if (!includeArchived) {
      list = list.filter(row => row.status?.toLowerCase() !== 'paid');
    }
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('payment-lines-print-area');
    if (!printContent) return;

    const htmlContent = `
      <html>
        <head>
          <title>Payment Lines Report</title>
          <style>
        ...
        <body>
          ...
          <h2 style="text-align: center; margin-top: 0; color: #1e293b;">Payment Lines Report</h2>
  ...
  return (
    <ReportLayout title="Payment Lines Report:">
      ...
      {loading ? (
        ...
        <Box id="payment-lines-print-area">
