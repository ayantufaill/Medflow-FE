...
const AgingReport = () => {
  ...
  const [showViewGeneratedStatements, setShowViewGeneratedStatements] =
    useState(false);
  ...
  const filteredReportData = useMemo(() => {
    let result = enrichedReportData.filter((row) => {
      // Flag Filter
      if (flagFilter === "with_flags" && (!row.flags || row.flags.length === 0))
        return false;
      if (flagFilter === "without_flags" && row.flags && row.flags.length > 0)
        return false;
      ...
      if (patientStatusFilter === "active" && row.isActive === false)
        return false;
      if (patientStatusFilter === "inactive" && row.isActive !== false)
        return false;
      ...
      if (owingFilter === "patient") {
        const ptTotal = Object.values(row.buckets || {}).reduce(
          (sum, b) => sum + (b.pt || 0),
          0,
        );
      ...
      if (owingFilter === "insurance") {
        const insTotal = Object.values(row.buckets || {}).reduce(
          (sum, b) => sum + (b.ins || 0),
          0,
        );
      ...
      if (billingDateFilter !== "any" && row.lastBilled) {
        ...
        const diffDays = Math.floor(
          (today - billedDate) / (1000 * 60 * 60 * 24),
        );
      ...
      if (arRangeFilter !== "any" && row.buckets) {
        if (
          arRangeFilter === "0-30" &&
          (row.buckets["0 - 30 days"]?.total || 0) <= 0
        )
          return false;
        if (
          arRangeFilter === "31-60" &&
          (row.buckets["31 - 60 days"]?.total || 0) <= 0
        )
          return false;
        if (
          arRangeFilter === "61-90" &&
          (row.buckets["61 - 90 days"]?.total || 0) <= 0
        )
          return false;
        if (arRangeFilter === ">90") {
          const over90 = [
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Aging_Report_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(
      "<html><head><title>Aging Report Table Only</title>",
    );
    ...
    printWindow.document.write(
      '.MuiCheckbox-root, input[type="checkbox"], button, .no-print { display: none !important; }',
    );
          ...
          <Box sx={{ mt: 2, borderTop: "2px solid #e0e0e0", pt: 2 }}>
            <Table size="small">
              <TableBody>
                <TableRow
                  sx={{
                    "& td": { fontSize: "0.75rem", border: "none", py: 0.2 },
                  }}
                >
                ...
                >
                  <TableCell
                    colSpan={(showFlags ? 1 : 0) + (hidePatientNames ? 1 : 2)}
                  ...
                  {agingBuckets.map((bucket) => (
                    <TableCell key={bucket} align="right">
                      $
                      {totals.buckets[bucket].total.toLocaleString("en-US", {
                ...
                >
                  ...
                  {agingBuckets.map((bucket) => (
                    <TableCell key={bucket} align="right">
                      $
                      {totals.buckets[bucket].pt.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </TableCell>
                  ...
                  <TableCell align="right" sx={{ fontWeight: 600 }}>
                    $
                    {totals.totalPt.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </TableCell>
                ...
                >
                  ...
                  {agingBuckets.map((bucket) => (
                    <TableCell key={bucket} align="right">
                      $
                      {totals.buckets[bucket].ins.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </TableCell>
                  ...
                  <TableCell align="right" sx={{ fontWeight: 600 }}>
                    $
                    {totals.totalIns.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </TableCell>
                ...
                <TableBody>
                  {[
                    ...
                    <TableRow
                      ...
                      onClick={() =>
                        handleDateSelect({ target: { value: report.date } })
                      }
                    >
                  ...
                  <TableBody>
                    {archivedLoading ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={2} align="center" sx={{ py: 3 }}>
                          <Typography variant="body2" color="text.secondary">
                            No report data found for this date.
                          </Typography>
                        </TableCell>
                      </TableRow>
                    ) : (
